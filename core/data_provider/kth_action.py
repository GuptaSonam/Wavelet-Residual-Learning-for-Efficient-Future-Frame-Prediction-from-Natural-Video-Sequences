...
class DataProcess:
    ...
    def load_data(self, paths, mode='train'):
        ...
        frames_np = np.expand_dims(frames_np,3)
        print(frames_np.shape)
        #data = np.zeros((frames_np.shape[0], self.image_width, self.image_width , 1))
        '''
        for i in range(len(frames_np)):
            temp = np.float32(frames_np[i, :, :,:])
            #data[i,:,:,0]=cv2.resize(temp,(self.image_width,self.image_width))
            #data[i,:,:,0]=cv2.resize(temp,(self.image_width,self.image_width))/255
            data[i,:,:,0]=cv2.resize(temp,(self.image_width,self.image_width)) / 127.5  - 1                        # normalizing between [-1, 1]
        '''
        print("there are " + str(frames_np.shape[0]) + " pictures")

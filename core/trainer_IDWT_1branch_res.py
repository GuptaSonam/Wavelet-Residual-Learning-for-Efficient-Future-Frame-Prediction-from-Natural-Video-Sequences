...
def test(model, test_input_handle, configs, itr):
    ...
    avg_mse = 0
    batch_id = 0
    img_mse, ssim, psnr, lpips_met = [], [], [], []
    #loss_fn = lpips.LPIPS(net='vgg', version='0.1')
    #loss_fn = loss_fn.to(configs.device)
    ...
    while (test_input_handle.no_batch_left() == False):
        ...
        for i in range(output_length):
            ...
            for b in range(configs.batch_size):
                ...
                im1 = lpips.im2tensor(pred_frm[b])
                im1 = im1.to(configs.device)
                im2 = lpips.im2tensor(real_frm[b])
                im2 = im2.to(configs.device)
                #print("im1, im2", im1.shape, im2.shape)    
                dist = loss_fn.forward(im1, im2)
                lpips_met[i] += dist
            
            for b in range(configs.batch_size):
                
                im1 = lpips.im2tensor(gx[b])
                im1 = im1.to(configs.device)
                im2 = lpips.im2tensor(x[b])
                im2 = im2.to(configs.device)
                #print("im1, im2", im1.shape, im2.shape)    
                dist = loss_fn.forward(im1, im2)
                lpips_met[i] += dist
        ...
        if batch_id <= configs.num_save_samples:
            ...
            for i in range(configs.total_length):
                ...
                img_gt = np.uint8((test_ims[0, i, :, :, :]+1.0)/2.0   * 255)
            ...
            for i in range(img_gen_length):
                ...
                img_pd = np.uint8((img_pd+1.0)/2.0  * 255)
    ...
    for i in range(configs.total_length - configs.input_length):
        print(ssim[i])
    '''
    lpips_met = np.asarray(lpips_met, dtype=np.float32) / (configs.batch_size * batch_id)
    print('ssim per frame: ' + str(np.mean(lpips_met)))
    fh.write('\n ssim per frame: ' + str(np.mean(lpips_met)))
    for i in range(configs.total_length - configs.input_length):
        print(lpips_met[i])   
    '''    

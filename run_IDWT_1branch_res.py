...
from core.models.model_factory_IDWT_1branch_res import Model
from core.utils import preprocess
import core.trainer_IDWT_1branch_res as trainer
...
def train_wrapper(model):
    ...
    train_input_handle, test_input_handle = datasets_factory.data_provider(
        args.dataset_name, args.train_data_paths, args.valid_data_paths, args.batch_size, 120, 160,
    ...
    for itr in range(1, args.max_iterations + 1):
        ...
        ims_orig = train_input_handle.get_batch()                        #(8,20,64,64,1)
        ims = np.zeros((args.batch_size, ims_orig.shape[1], args.img_width, args.img_width,1))
        for i in range(args.batch_size):
            for j in range(ims_orig.shape[1]):
                temp = np.float32(ims_orig[i, j, :, :,0])
                ims[i,j,:,:,0]=cv2.resize(temp,(args.img_width,args.img_width)) / 127.5  - 1  
